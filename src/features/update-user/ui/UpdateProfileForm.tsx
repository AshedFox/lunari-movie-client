...
import { updateUserScheme } from '../model';
import { useRouter } from 'next/navigation';
import { updateProfileAction, updateAvatarAction } from '../api/actions';
import {
  Form,
  FormControl,
  FormField,
...
export const UpdateProfileForm = ({ user, countries }: Props) => {
  ...
  const onSubmit = async (input: UpdateUserInput) => {
    toast.promise(updateProfileAction(input), {
  ...
  const onFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length === 1) {
      setIsUploading(true);
      toast.promise(updateAvatarAction(e.target.files[0]), {
