...
export const MovieListEditor = ({
  ...
  const handleRemove = (id: string) => {
    startTransition(() => {
      toast.promise(deleteCollectionMovieAction(collectionId, id), {
        loading: 'Loading...',
        error: (e) => (e instanceof Error ? e.message : 'Something went wrong'),
        success: (data) => {
          setMovies((prev) => prev.filter((m) => m.id !== data.movieId));
          return 'Successfully removed movie from collection.';
        },
      });
    });
  };
  ...
  const handleAdd = (movie: Movie) => {
    if (!movies.some((m) => m.id === movie.id)) {
      startTransition(() => {
        toast.promise(addCollectionMovieAction(collectionId, movie.id), {
          loading: 'Loading...',
          error: (e) =>
            e instanceof Error ? e.message : 'Something went wrong',
          success: () => {
            setMovies((prev) => [...prev, movie]);
            return 'Successfully added movie to collection.';
          },
        });
      });
    }
